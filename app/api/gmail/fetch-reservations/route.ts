...
export async function GET() {
  try {
    ...
    // Gmailクライアントを取得
    const gmailClient =
      await GmailClient.fromUserId(userId);
    ...
    const afterDate =
      settings?.gmailLastSyncAt || sixtyDaysAgo;

    // Gmailから予約確認メールを検索
    const messages =
      await gmailClient.searchReservationEmails(
        50,
        afterDate
      );
    ...
    // メールをパース
    const parsedResults =
      await parseReservationEmails(messages);
    const validResults =
      filterValidReservations(parsedResults);
    const uniqueResults =
      deduplicateReservations(validResults);

    // 予約を保存
    const newReservations = await saveReservations(
      userId,
      uniqueResults
    );
    ...
    console.error(
      'Error fetching Gmail reservations:',
      error
    );
    return NextResponse.json(
      {
